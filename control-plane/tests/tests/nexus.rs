...
async fn create_nexus_sizes() {
    ...
    for size_mb in &vec![6, 10, 100] {
        ...
        let disk =
            || format!("malloc:///disk?size_mb={}", size / (1024 * 1024));
...
async fn create_nexus_local_replica() {
    ...
    let replica = format!("bdev:///{}", Cluster::replica(0, 0));
    cluster
        .rest_v0()
        .create_nexus(v0::CreateNexus {
            node: cluster.node(0),
...
async fn create_nexus_replicas() {
    ...
    let local = format!("bdev:///{}", Cluster::replica(0, 0));
    let remote = cluster
        .rest_v0()
...
async fn create_nexus_replica_not_available() {
    ...
    cluster
        ...
        .await
        .expect_err("One replica is not present so nexus can't be created");
}
